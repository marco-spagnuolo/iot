...
func iznizialize(idUser string) {

	err := os.Setenv("DISCOVERY_AS_LOCALHOST", "true")
	if err != nil {
		fmt.Errorf("error setting DISCOVERY_AS_LOCALHOST environemnt variable: %v", err)
	}
	...
	if err != nil {
		fmt.Errorf("failed to create wallet: %v", err)
	}
	...
	if !wallet.Exists(idUser) {
		...
		if err != nil {
			fmt.Errorf("failed to populate wallet contents: %v", err)
		}
	}
	...
	if err != nil {
		fmt.Errorf("failed to connect to gateway: %v", err)
	}
	...
	if err != nil {
		fmt.Errorf("failed to get network: %v", err)
	}
